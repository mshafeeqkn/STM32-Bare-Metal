...
void write_eeprom_data(uint8_t chip, uint8_t addr, uint8_t* data, uint16_t len) {
    ...
    while(len) {
        ...
        // Re-calculate the next page start and
        // end addresses for next iteration.
        addr += (end - start);
        start += (end - start);
        if(len > 16) {
            end += 16;
        ...
        // When first 512 bytes completed, next
        // address space is current I2C address + 1
        if(addr == 0 && chip == 0x50) {
            chip++;
        }

        // Give 5ms delay at 8MHz clock
        delay(i10000);
    }
}
